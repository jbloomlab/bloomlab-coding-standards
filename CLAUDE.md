...
- Standard layout, at the repository root: `config.yml` (configuration), `Snakefile` (workflow), `rules/` (`.smk` files), `scripts/`, `notebooks/`, `data/` (all input data), `results/` (all generated output), `environment.yml` (unless the project uses a lab pipeline's environment), `pyproject.toml` (wherever there is Python; holds the linter and formatter configuration), `.gitignore`, `README.md`, `CLAUDE.md`. As needed: `auspice/`, `non-pipeline_analyses/`, `LICENSE`, a cluster submission script, and lab pipeline submodules.
...
- Analyses run outside the main pipeline go in `non-pipeline_analyses/`, each with its own configuration, conda environment, and README. Prefer that name; where a project uses another (`manual_analyses`, `scratch_notebooks`), it is acceptable, but suggest renaming to `non-pipeline_analyses`. Do not let this directory bloat: an analysis there must not re-implement what the pipeline already does, and one that becomes load-bearing is migrated into the pipeline as a rule plus a script — porting only the part the pipeline does not already provide, rather than moving a notebook across whole.
...
- **A value reconstructed from results is not an observation.** Where a column was filled in by inferring it from the very data it will be used to analyze, say so where it is recorded and have it confirmed against the bench records.
...
- Where the config genuinely repeats a value or a block across entries, factor it with a YAML anchor rather than copy-pasting it. The lab also uses YTE templating (`__use_yte__: true`) to generate a set of entries from one template; reach for that only when an anchor will not do, since a templated config is harder to read. Deriving the repetition from the data is better than either.
...
- Ignore `results/` by default in `.gitignore`, then re-include the key scientific outputs — compact, human-readable CSV/TSV/YAML/TXT — so someone can read the main results without running the pipeline. Do not track bulk, intermediate, or large regenerable output.
...
- One conceptual change per commit and per pull request. Do not combine conceptually unrelated changes.
