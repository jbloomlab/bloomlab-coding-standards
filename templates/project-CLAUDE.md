...
<add optional project-specific content: coding conventions only (how to run the pipeline and tests, rules that extend or override the lab rules above, known pitfalls, tool limitations worked around). Do not describe the project, individual Snakemake rules, or configuration keys — those belong in `README.md` and in comments in `config.yml`. Keep this file short: no more than a few hundred lines.>
